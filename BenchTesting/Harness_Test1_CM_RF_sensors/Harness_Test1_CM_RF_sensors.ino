...
const int data_change_interval = 2000;             // delay before changing data bytes; milliseconds

unsigned long tick, tock, tx_count = 0;
int log_after = 8;                               // send log msg every (data_change_interval)*(log_after) milliseconds
int change_count = 0;                             // counts up to log_after
int sensor_data = 0;

unsigned char TEC, EFLG = 0;                      // Data to report node status stored in these

UTFR_CAN_CORNER_RF NODE(HW_PIN_CS_TEST);          // Create CAN node object
  ...
  if ((tock - tick) > data_change_interval){
      ...
      switch(change_count)
      {
        case 0:
          ...
          NODE.setField(CAN_MSG_20Hz_1, RF_20Hz_1_F, sensor_data);
        case 1:
      ...
      Serial.print("Sensor "); Serial.print(change_count+1); Serial.println(" sending.");*/

      sensor_data = analogRead(HW_PIN_SENSOR_8);
      NODE.setField(CAN_MSG_20Hz_1, RF_20Hz_1_F, sensor_data);

      sensor_data = analogRead(HW_PIN_SENSOR_9);
      NODE.setField(CAN_MSG_20Hz_2, RF_20Hz_2_F, sensor_data);
      
      /*
      if (NODE.getField(CAN_MSG_20Hz_1, RF_20Hz_1_F) == 6969){
          NODE.setField(CAN_MSG_20Hz_1, RF_20Hz_1_F, 1111);
          NODE.setField(CAN_MSG_20Hz_2, RF_20Hz_2_F, 1111);
          NODE.setField(CAN_MSG_50Hz, RF_50Hz_F, 1111);
      }
      else{
          NODE.setField(CAN_MSG_20Hz_1, RF_20Hz_1_F, 6969);
          NODE.setField(CAN_MSG_20Hz_2, RF_20Hz_2_F, 6969);
          NODE.setField(CAN_MSG_50Hz, RF_50Hz_F, 6969);
      } */

      if (change_count >= log_after) {        // log mcp2515 Transmit ERFor Count (TEC) and ERFor Flags (EFLG)  
          /*
