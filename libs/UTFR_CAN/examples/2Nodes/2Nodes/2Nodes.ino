...
UTFR_CAN NODES(HW_PIN_CS_TEST1, HW_PIN_CS_TEST2);       // Create CAN NODES object
...
// --- NODES 1 ---
  ...
  NODES.sendMsg(CAN_MSG_RF0, 1);      // Send message by name, name defined in UTFR_CAN_version.h
// --- NODES 2 ---
  NODES.sendAll(2);                   // Send all messages defined in UTFR_CAN_version.h
  NODES.sendMsg(CAN_MSG_RF0, 2);      // Send message by name, name defined in UTFR_CAN_version.h
// --- Both ---
  NODES.sendAll(3);
  NODES.sendMsg(CAN_MSG_RF0, 3);
...
// ----------- GETTING/SETTING MSG DATA ---------------
  rf_out_tire_temp = NODES.getField(CAN_MSG_RF0, RF_OUT_TIRE_TEMP_F);       // Get field data by name
  NODES.setField(CAN_MSG_RF0, RF_OUT_TIRE_TEMP_F, rf_out_tire_temp+1);      // Set field data by name
