...
void msgSendISR(UTFR_CAN& node)
{
    //Store data from RF0 sensors:
    node.setField(CAN_MSG_RF0, RF_OUT_TIRE_TEMP_F, analogRead(HW_PIN_RF_OUT_TIRE_TEMP));
    node.setField(CAN_MSG_RF0, RF_CTR_TIRE_TEMP_F, analogRead(HW_PIN_RF_CTR_TIRE_TEMP));
    node.setField(CAN_MSG_RF0, RF_INR_TIRE_TEMP_F, analogRead(HW_PIN_RF_INR_TIRE_TEMP));
    node.setField(CAN_MSG_RF0, RF_ROTOR_TEMP_F, analogRead(HW_PIN_RF_ROTOR_TEMP));

    //Storing data from RF1 sensors:
    node.setField(CAN_MSG_RF1, RF_DAMP_POT_F, analogRead(HW_PIN_RF_DAMPER_POT));
    node.setField(CAN_MSG_RF1, FW_STRAIN_TIP_F , analogRead(HW_PIN_FW_STRAIN_GAUGE_TIP));
    node.setField(CAN_MSG_RF1, STEER_ANG_F, analogRead(HW_PIN_STEERING_ANGLE));

    //Send sensor data messages
    node.sendMsg(CAN_MSG_RF0);
    node.sendMsg(CAN_MSG_RF1);
}
