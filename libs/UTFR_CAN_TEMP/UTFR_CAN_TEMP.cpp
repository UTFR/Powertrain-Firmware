...
UTFR_CAN_TEMP::UTFR_CAN_TEMP(uint8_t CS1)
{   
...
UTFR_CAN_TEMP::UTFR_CAN_TEMP(uint8_t CS1, uint8_t CS2)
{   
...
void UTFR_CAN_TEMP::begin(uint8_t busSpeed)
{   
...
void UTFR_CAN_TEMP::receiveMsgs()               
{
...
void UTFR_CAN_TEMP::setFilters_permitAll()
{
...
void UTFR_CAN_TEMP::setFilters_permitNone()
{
    for (uint8_t i=0; i<CAN_MASK_FILTER_COUNT; i++)
    {   
        if (i < 2)
...
// Starts node - must be called before any other functions
void UTFR_CAN_TEMP::begin(uint8_t busSpeed, uint8_t nodeNumber)
{   
    bool validSpeed = false; 
...
// Send all messages where .isTx == true
void UTFR_CAN_TEMP::sendAll(uint8_t nodeNumber)
{
    uint16_t canID;
...
// Send message by name
void UTFR_CAN_TEMP::sendMsg(CAN_msgNames_E msgName, uint8_t nodeNumber)
{
    uint16_t canID;
    canID = static_cast<uint16_t>(_CAN_msgArray[msgName].msgID & 0x0FFFF);   // shortening unsigned long to 16 bits to save memory
...
void UTFR_CAN_TEMP::receiveMsgsCommonFcn(unsigned long canID, uint8_t dataLength, uint8_t buf[8])
{
    for (uint8_t msgIdx=0; msgIdx<CAN_MSG_COUNT; msgIdx++)          // find matching ID and update that msgData with received data 
    {                                                               // iterating should be fine as each MCU handles very few unique msgs (<10)
        if (canID == _CAN_msgArray[msgIdx].msgID){
...
// Receive pending messages and update message data in-place (in _CAN_msgArray)
void UTFR_CAN_TEMP::receiveMsgs(uint8_t nodeNumber)               
{
        uint8_t dataLength = 0;
        uint8_t buf[8];
        unsigned long canID;
...
// Set masks and filters according to CAN_filterArray
void UTFR_CAN_TEMP::setFilters(uint8_t nodeNumber)
{
    for (uint8_t i=0; i<CAN_MASK_FILTER_COUNT; i++)
    {   
        if (i < 2)
...
// Set masks and filters so that all messages sent by other nodes are received
void UTFR_CAN_TEMP::setFilters_permitAll(uint8_t nodeNumber)
{
    // NODE 1 ------------------------------------------------------
    if (nodeNumber == 1)
    {
...
// Set masks and filters so that this node receives NO messages
void UTFR_CAN_TEMP::setFilters_permitNone(uint8_t nodeNumber)
{
    for (uint8_t i=0; i<CAN_MASK_FILTER_COUNT; i++)
    {   
        if (i < 2)
...
* @param[in]        fieldName  Name of field, defined at top of UTFR_CAN_TEMP_version.h
*
* @return    unsigned long     Data of gotten field
*
*/
unsigned long UTFR_CAN_TEMP::getField(CAN_msgNames_E msgName, uint8_t fieldName)
{
...
*
*/
void UTFR_CAN_TEMP::setField(CAN_msgNames_E msgName, uint8_t fieldName, long fieldData)
{
...
/*!
* @brief    Calculate the checksum value of the given message and set to the last byte.
*
* @param[in]        msgName    Name of message, defined in _CAN_msgArray
*/
void UTFR_CAN_TEMP::calculateChecksum(CAN_msgNames_E msgName)
{
    long sum = 0;
    sum += _CAN_msgArray[msgName].msgID;          //Sum Message ID
    sum += 8;                                     //Sum Message Length
    for (uint8_t dataIdx=0; dataIdx<7; dataIdx++)
    {
      sum += _CAN_msgArray[msgName].msgData[dataIdx];
    }
    setField(msgName, 8, sum);
}



// Prints all data stored in a given message (Note: will be in decimal format)
void UTFR_CAN_TEMP::printMsgData(CAN_msgNames_E msgName)
{
