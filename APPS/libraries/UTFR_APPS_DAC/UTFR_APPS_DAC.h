...
class UTFR_APPS_DAC : public MCP4911 {                       // APPS_DAC inherits all of MCP4911's methods and properties
    ...
    protected:
        ...
        const int _kAPPS_1_HIGH = 4.75;
        const int _kAPPS_2_HIGH = 4.275;
        const int _kAPPS_1_LOW = 1.25;
        const int _kAPPS_2_LOW = 1.125;
        ...
        const float _kOUTPUT_DEVIATION = 0.03;           // abitrary value for time_now, will be determined experimentally
        const long _kBASE_TIME = -1;

        int _APPS_1_in = 0;
        int _APPS_2_in = 0;                              // 0 - 1023
        int _APPS_output = 0;
        float _APPS_out_verify = 0;                      // 0 - 1023
