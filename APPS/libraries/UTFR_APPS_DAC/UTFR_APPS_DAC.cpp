...
void UTFR_APPS_DAC::processThrottlePosition()
{
    ...
    // set flags
    _throttle_good = getDeviation(_APPS_1_in, _APPS_2_in) < _kTHROTTLE_MAX_DEVIATION;
...
{
    _APPS_output = (_APPS_1_throttle + _APPS_2_throttle) / 2; // send out the mean of both inputs
...
void UTFR_APPS_DAC::shutDown()
{
    ...
    // ADD COMMS WITH MEGA HERE (Tell mega to disconnect from chassis ground)
}